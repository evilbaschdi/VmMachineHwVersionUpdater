    ...
    [NotNull] IReloadReactiveCommand reloadReactiveCommand) : ReactiveCommandUnitTask, IUpdateAllReactiveCommand
{
    ...
    private readonly IReloadReactiveCommand _reloadReactiveCommand =
        reloadReactiveCommand ?? throw new ArgumentNullException(nameof(reloadReactiveCommand));
    ...
    public override async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        var localList = _load.Value.VmDataGridItemsSource
                             .Where(vm => vm.HwVersion != innerVersion && vm.MachineType == MachineType.Vmx).AsParallel();
